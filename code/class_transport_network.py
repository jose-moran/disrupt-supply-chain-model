...
class TransportNetwork(nx.Graph):
    ...
    def add_transport_node(self, node_id, all_nodes_data):
        #used in add_transport_edge_with_nodes
    ...
    def add_transport_edge_with_nodes(self,
                                      edge_id,
                                      all_edges_data,
                                      all_nodes_data):
        # Selecting data
        edge_attributes = ["id", "type", "surface",
                           "geometry", "class", "km",
                           "special", "name", "multimodes",
                           "capacity", "cost_per_ton", "travel_time",
                           "time_cost", "cost_travel_time",
                           "cost_variability", "agg_cost"]
            ...
            if len(segment) == 2:  # only edges have costs
                if self[segment[0]][segment[1]]['type'] != 'virtual':
                    ...
                    cost_per_ton += (surface == 'paved')*self.graph['unit_cost']['roads']['paved'] +\
                        (surface == 'unpaved') * \
                        self.graph['unit_cost']['roads']['unpaved']
    ...
    def giveRouteCostWithCongestion(self, route):
        ...
        for segment in route:
            if len(segment) == 2:  # only edges have costs
                if self[segment[0]][segment[1]]['type'] != 'virtual':
                    time_cost += self[segment[0]][segment[1]]['cost_variability'] + self[segment[0]
                                                                                         ][segment[1]]['cost_travel_time'] * (1 + self[segment[0]][segment[1]]['congestion'])
        return time_cost
    ...
    def giveCongestionCostOfTime(self, route):
        ...
        for segment in route:
            if len(segment) == 2:  # only edges have costs
                if self[segment[0]][segment[1]]['type'] != 'virtual':
                    congestion_time_cost += self[segment[0]][segment[1]
                                                             ]['cost_travel_time'] * self[segment[0]][segment[1]]['congestion']
    ...
    def defineWeights(self, route_optimization_weight):
        ...
        for edge in self.edges:
            self[edge[0]][edge[1]]['weight'] = self[edge[0]
                                                    ][edge[1]][route_optimization_weight]
            self[edge[0]][edge[1]]['capacity_weight'] = self[edge[0]
                                                             ][edge[1]][route_optimization_weight]
            self[edge[0]][edge[1]]['road_weight'] = self[edge[0]
                                                         ][edge[1]][route_optimization_weight]
            self[edge[0]][edge[1]]['intl_road_shv_weight'] = self[edge[0]
                                                                  ][edge[1]][route_optimization_weight]
            self[edge[0]][edge[1]]['intl_road_vnm_weight'] = self[edge[0]
                                                                  ][edge[1]][route_optimization_weight]
            self[edge[0]][edge[1]]['intl_rail_weight'] = self[edge[0]
                                                              ][edge[1]][route_optimization_weight]
            self[edge[0]][edge[1]]['intl_river_weight'] = self[edge[0]
                                                               ][edge[1]][route_optimization_weight]
        ...
        for firm in firm_list:
            if firm.odpoint != -1:
                try:
                    ...
                    logging.error(
                        'Transport network has no node numbered: '+str(firm.odpoint))
    ...
    def provide_shortest_route(self, origin_node, destination_node, route_weight):
        ...
        if (origin_node not in self.nodes):
            logging.warning("Origin node "+str(origin_node) +
                            " not in the available transport network")
            return None
            ...
            sp = nx.shortest_path(
                self, origin_node, destination_node, weight=route_weight)
            route = [[(sp[0],)]] + [[(sp[i], sp[i+1]), (sp[i+1],)]
                                    for i in range(0, len(sp)-1)]
            route = [item for item_tuple in route for item in item_tuple]
            return route

        else:
            logging.warning("There is no path between " +
                            str(origin_node)+" and "+str(destination_node))
            return None
    ...
    def sum_indicator_on_route(self, route, indicator, detail_type=False):
        ...
        if detail_type:
            ...
            # print(details)
            detail_per_cat = details.groupby(['type', 'multimodes', 'special'])[
                indicator].sum()
    ...
    def get_undisrupted_network(self):
        available_nodes = [
            node for node in self.nodes if self.node[node]['disruption_duration'] == 0]
        available_subgraph = self.subgraph(available_nodes)
        available_edges = [
            edge for edge in self.edges if self[edge[0]][edge[1]]['disruption_duration'] == 0]
    ...
    def transport_shipment(self, commercial_link):
        ...
        self.update_load_on_route(
            route_to_take, commercial_link.delivery_in_tons)
    ...
    def evaluate_normal_traffic(self, sectorId_to_volumeCoef=None):
        ...
        for edge in self.edges():
            if self[edge[0]][edge[1]]['type'] == 'virtual':
                continue
            self[edge[0]][edge[1]]['traffic_normal'] = self[edge[0]
                                                            ][edge[1]]['traffic_current']
        ...
        for edge in self.edges():
            ...
            if sectorId_to_volumeCoef is not None:
                ...
                for sector_id in sectors_causing_congestion:
                    ...
                    volume += sectorId_to_volumeCoef[sector_id] * \
                        sum(list_montetary_flows)
