...
def load_transport_data(filepaths: list[str],
    ...
    any_edge = False
    any_node = False
    if transport_mode in ["roads", "railways",
                          "waterways", "maritime"]:
        any_node = True
    if transport_mode in ["roads", "railways",
                          "waterways", "maritime", "multimodal"]:
        any_edge = True

    # Load nodes
    if any_node:
        nodes = gpd.read_file(filepaths[transport_mode+'_nodes'])
        # if 'index' in nodes.columns: #remove any column named "index".
        #  Seems to create issues
    ...
    if any_edge:
        ...
        # if 'index' in edges.columns: #remove any column named "index".
        #  Seems to create issues
        ...
        if (transport_mode == "roads") and additional_roads:
            ...
            # if 'index' in new_road_edges.columns:
            # #remove any column named "index". Seems to create issues
        ...
        # Compute how much it costs to transport one USD worth of goods on
        #  each edge
        edges = comp_cost_ttime_edges(
            edges, transport_params, edge_type=transport_mode)
        ...
        # / periods[time_resolution]
        # if (transport_mode == "roads"):
        #     edges.loc[edges['surface']=="unpaved", "capacity"] = 100000*52 \
        # / periods[time_resolution]
...
def comp_cost_ttime_edges(edges: gpd.GeoDataFrame,
    ...
    special_custom = edges['special'].str.contains("custom", na=False)
    edges.loc[special_custom, "cost_per_ton"] = \
        edges.loc[special_custom, "type"].map(transport_params['custom_cost'])
...
def assign_road_costs(edges: gpd.GeoDataFrame, cost_per_ton: dict):
    ...
    edges['cost_per_ton'] = edges['km'] * (
        (edges['surface'] == 'unpaved') * unpaved_cost +
        (edges['surface'] == 'paved') * paved_cost
    )
...
def assign_all_transport_costs(edges: gpd.GeoDataFrame,
    ...
    if edge_type == "roads":
        assign_road_costs(edges, cost_per_ton)
    elif edge_type in ['railways', 'waterways', 'maritime']:
        edges['cost_per_ton'] = edges['km'] * cost_per_ton[edge_type]

    elif edge_type == "multimodal":
        edges['cost_per_ton'] = edges['multimodes'].map(
...
def get_idx_closest_pt(point, df_with_points):
    distList = [point.distance(item)
                for item in df_with_points['geometry'].tolist()]
...
def filter_sector(sector_table: pd.DataFrame,
    ...
    if len(filtered_sectors) == 0:
        raise ValueError(
            "The cutoff value is so high that it filtered out all sectors")
...
def resc_nb_firms(fpath_district_sect_imptance: str,
    ...
    if isinstance(sectors_to_include, list):
        ...
        raise ValueError(
            "'sectors_to_include' should be a list of string or 'all'")
    ...
    table_odpoints = extrct_0dpt_tbl_frm_transp_nodes(transport_nodes)
    od_sector_table = pd.merge(
        table_odpoints, filtered_district_sector_table, how='inner', on='district')
    od_sector_table['importance'] = od_sector_table['importance'] / \
        od_sector_table['nb_points_same_district']
    logging.info('Initial nb of OD points: '+str(table_odpoints.shape[0]))
    logging.info('Filtered OD points: ' +
                 str(od_sector_table["odpoint"].nunique()))
    ...
    col_to_keep = ['id', 'sector', 'sector_type',
                   'odpoint', 'importance', 'district', 'long', 'lat']
...
def create_firms(firm_table, keep_top_n_firms=None, reactivity_rate=0.1, utilization_rate=0.8):
    """Create the firms
...
def load_tech_coeffs(firm_list, filepath_tech_coef, io_cutoff=0.1, import_sector_name=None):
    ...
    if import_sector_name:
        tech_coef_matrix = tech_coef_matrix.loc[sector_present + [
            import_sector_name], sector_present]
    else:
        tech_coef_matrix = tech_coef_matrix.loc[sector_present, sector_present]

    # Load input mix
    for firm in firm_list:
        firm.input_mix = tech_coef_matrix.loc[tech_coef_matrix.loc[:,
                                                                   firm.sector] != 0, firm.sector].to_dict()

    return firm_list


def load_invents(firm_list, inventory_duration_target=2,
    ...
    if isinstance(inventory_duration_target, int):
        for firm in firm_list:
            firm.inventory_duration_target = {
                input_sector: inventory_duration_target for input_sector in firm.input_mix.keys()}

    elif inventory_duration_target == 'inputed':
        dic_sector_inventory = pd.read_csv(filepath_inventory_duration_targets).set_index(
            ['buying_sector', 'input_sector'])['inventory_duration_target'].to_dict()
        for firm in firm_list:
            firm.inventory_duration_target = {
        ...
        raise ValueError(
            "Unknown value entered for 'inventory_duration_target'")
    ...
    if isinstance(extra_inventory_target, int):
        if isinstance(inputs_with_extra_inventories, list) and (buying_sectors_with_extra_inventories == 'all'):
            for firm in firm_list:
                firm.inventory_duration_target = {
                    input_sector: firm.inventory_duration_target[input_sector] +
                    extra_inventory_target
...
def load_ton_usd_equiv(sector_table, firm_list, country_list):
    ...
    # should disappear, we want only sector_to_usdPerTon
    return firm_list, country_list, sector_to_usdPerTon


def resc_mon_vals(values, time_resolution="week", target_units="mUSD", input_units="USD"):
...
def create_countries(filepath_imports, filepath_exports, filepath_transit_matrix,
    ...
    time_resolution : see resc_mon_vals
    target_units : see resc_mon_vals
    input_units : see resc_mon_vals

    Returns
    -------
    list of Countries
    """
    import_table = resc_mon_vals(
        pd.read_csv(filepath_imports, index_col=0),
        time_resolution=time_resolution,
        target_units=target_units,
        input_units=input_units
    )
    export_table = resc_mon_vals(
        pd.read_csv(filepath_exports, index_col=0),
        time_resolution=time_resolution,
        target_units=target_units,
        input_units=input_units
    )
    transit_matrix = resc_mon_vals(
    ...
    if isinstance(countries_to_include, list):
        ...
        transit_matrix = transit_matrix.loc[countries_to_include,
        ...
        raise ValueError(
            "'countries_to_include' should be a list of string or 'all'")
...
def def_final_demand(firm_table, od_table,
    ...
    ----------
    firm_table : pandas.DataFrame
        firm_table from resc_nb_firms function

    od_table: pandas.DataFrame
        od_table from resc_nb_firms function
    ...
    od_table = pd.merge(od_table, population_per_district,
                        on='district', how='left')
    od_table['population'] = od_table['population'] / \
        od_table['nb_points_same_district']
    od_table['perc_population'] = od_table['population'] / \
        od_table['population'].sum()
    ...
    firm_table['final_demand_weight'] = firm_table['perc_population'] / \
        firm_table['nb_firms_same_point_same_sector']
    ...
    firm_table['final_demand_weight'] = firm_table['final_demand_weight'] / \
        firm_table['sector'].map(firm_table.groupby('sector')[
                                 'final_demand_weight'].sum())

    # Check that weight sum up to 1
    sum_of_sectoral_final_demand_weight = firm_table.groupby(
        'sector')['final_demand_weight'].sum()
    if ((sum_of_sectoral_final_demand_weight-1.0) > 1e-6).any():
        logging.warning('The final demand of some sectors is problematic: ' +
                        str(sum_of_sectoral_final_demand_weight))
    ...
    final_demand_per_sector["final_demand"] = resc_mon_vals(
    ...
    firm_table['final_demand'] = firm_table['sector'].map(
        final_demand_per_sector.set_index('sector')['final_demand'])
    firm_table['final_demand'] = firm_table['final_demand'] * \
        firm_table['final_demand_weight']
    logging.info("Total final demand per "+time_resolution+" is " +
                 "{:.01f} ".format(firm_table['final_demand'].sum())+target_units)
    return firm_table


def create_hholds(firm_table):
    """Create Households objecvt

    :param firm_table: firm_table from resc_nb_firms and def_final_demand functions
    :type firm_table: pandas.DataFrame

    :return: Households object
    """
    households = Households()
    households.final_demand_per_sector = firm_table.groupby(
        'sector')['final_demand'].sum().to_dict()
    households.purchase_plan = firm_table[['id', 'final_demand']].set_index('id')[
        'final_demand'].to_dict()
    households.extra_spending_per_sector = {
        key: 0 for key, val in households.final_demand_per_sector.items()}
    return households


def extract_edg_list(graph):
    dic_commercial_links = nx.get_edge_attributes(graph, "object")
    dic_commercial_links = {key: value for key, value in dic_commercial_links.items(
    ) if (isinstance(key[0], Firm) and isinstance(key[1], Firm))}
...
def def_dirspt_list(disruption_analysis, transport_network,
    ...
    if disruption_analysis is None:
        raise ValueError(
            "'disruption_analysis' is None, cannot define the disruption list")
        ...
        if isinstance(disruption_analysis['nodeedge_tested'][0], list):
            ...
            if disruption_analysis['disrupt_nodes_or_edges'] == "nodes":
                disruption_list = nodes.loc[
                    nodes[disruption_analysis['identified_by']].isin(
                        disruption_analysis['nodeedge_tested']),
                    'id'
                ].tolist()
            elif disruption_analysis['disrupt_nodes_or_edges'] == "edges":
                disruption_list = edges.loc[
                    edges[disruption_analysis['identified_by']].isin(
                        disruption_analysis['nodeedge_tested']),
                    'id'
                ].tolist()
            else:
                raise ValueError(
                    "disruption_analysis['disrupt_nodes_or_edges'] should be 'nodes' or 'edges'")
    ...
    # if 'all' is indicated, need to retrieve all node or edge ids from the transport network
        ...
        if disruption_analysis['disrupt_nodes_or_edges'] == "nodes":
            ...
            disruption_list = list(nx.get_edge_attributes(
                actual_transport_network, 'id').values())
        else:
            raise ValueError(
                "disruption_analysis['disrupt_nodes_or_edges'] should be 'nodes' or 'edges'")
    ...
    # if a filepath is given, then load it. It is the list
        ...
        if disruption_analysis['nodeedge_tested'][-4:] == ".csv":
            ...
            raise ValueError(
                "If defining a path to a file in 'nodeedge_tested', it should be a csv")

    else:
        raise ValueError(
            "'nodeedge_tested' should be list of node/edge ids, a path to a csv file, or 'all'")
...
def export_sc_netwrk_summary(sc_graph, firm_list, export_folder):
    """
    ...
    for edge in sc_graph.edges:
        nb_F2F_links += int(isinstance(edge[0], Firm)
                            and isinstance(edge[1], Firm))
        nb_F2H_lins += int(isinstance(edge[0], Firm)
                           and isinstance(edge[1], Households))
        nb_C2F_links += int(isinstance(edge[0], Country)
                            and isinstance(edge[1], Firm))
        nb_F2C_links += int(isinstance(edge[0], Firm)
                            and isinstance(edge[1], Country))
        nb_C2C_links += int(isinstance(edge[0], Country)
                            and isinstance(edge[1], Country))

    nb_clients_known_by_firm = sum([sum([1 if isinstance(
        client_id, int) else 0 for client_id in firm.clients.keys()]) for firm in firm_list])
    nb_suppliers_known_by_firm = sum([sum([1 if isinstance(
        supplier_id, int) else 0 for supplier_id in firm.suppliers.keys()]) for firm in firm_list])
    res = {
    ...
    pd.Series(res).to_csv(os.path.join(
        export_folder, "sc_network_summary.csv"))
