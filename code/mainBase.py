...
logging.info('Simulation '+timestamp+' starting using ' +
...
extra_road_log = ""
if extra_roads:
    pickle_suffix = '_'.join([mode[:3]
                             for mode in transport_modes])+'ext_pickle'
...
if sys.argv[1] == "0":
    ...
    T, transport_nodes, transport_edges = create_transp_netwrk(
        transport_modes, filepaths, transport_params)
    logging.info('Transport network'+extra_road_log+' created.')
    pickle.dump(T, open(os.path.join('tmp', pickle_transNet_filename), 'wb'))
    pickle.dump(transport_edges, open(
        os.path.join('tmp', pickle_transEdg_filename), 'wb'))
    pickle.dump(transport_nodes, open(
        os.path.join('tmp', pickle_transNod_filename), 'wb'))
    logging.info('Transport network saved in tmp folder: ' +
                 pickle_transNet_filename)
else:
    T = pickle.load(open(os.path.join('tmp', pickle_transNet_filename), 'rb'))
    transport_edges = pickle.load(
        open(os.path.join('tmp', pickle_transEdg_filename), 'rb'))
    transport_nodes = pickle.load(
        open(os.path.join('tmp', pickle_transNod_filename), 'rb'))
    logging.info('Transport network'+extra_road_log +
                 ' generated from temp file.')
# Generate weight
...
km_per_mode = pd.DataFrame({"km": nx.get_edge_attributes(
    T, "km"), "type": nx.get_edge_attributes(T, "type")})
...
logging.info('Nb of nodes: '+str(len(T.nodes)) +
             ', Nb of edges: '+str(len(T.edges)))
# Export transport network
transport_nodes.to_file(os.path.join(
    exp_folder, "transport_nodes.geojson"), driver='GeoJSON')
transport_edges.to_file(os.path.join(
    exp_folder, "transport_edges.geojson"), driver='GeoJSON')
...
filtered_sectors = filter_sector(sector_table, cutoff=cutoff_sector_output['value'],
...
firm_list = create_firms(firm_table, nb_firms,
                         reactivity_rate, utilization_rate)
n = len(firm_list)
present_sectors = list(set([firm.sector for firm in firm_list]))
present_sectors.sort()
flow_types_to_export = present_sectors + \
    ['domestic_B2B', 'transit', 'import', 'export', 'total']
...
import_code = sector_table.loc[sector_table['type']
                               == 'imports', 'sector'].iloc[0]
firm_list = load_tech_coeffs(
    firm_list, filepaths['tech_coef'], io_cutoff, import_code)
logging.info('Technical coefficient loaded. io_cutoff: '+str(io_cutoff))

# Loading the inventories
firm_list = load_invents(firm_list, inventory_duration_target=inventory_duration_target,
                         ...
                         min_inventory=1)
logging.info('Inventory duration targets loaded, inventory_duration_target: ' +
             str(inventory_duration_target))
...
logging.info('Creating country_list. Countries included: ' +
             str(countries_to_include))
country_list = create_countries(filepaths['imports'], filepaths['exports'],
...
logging.info('Country_list created: ' +
             str([country.pid for country in country_list]))
...
firm_list, country_list, sector_to_usdPerTon = load_ton_usd_equiv(
    sector_table, firm_list, country_list)

# Create agents: Households
logging.info(
    'Defining the final demand to each firm. time_resolution: '+str(time_resolution))
firm_table = def_final_demand(firm_table, odpoint_table,
...
households = create_hholds(firm_table)
logging.info('Households created')


# Create supply chain network
logging.info('The supply chain graph is being created. nb_suppliers_per_input: ' +
             str(nb_suppliers_per_input))
G = nx.DiGraph()

logging.info(
    'Tanzanian households are selecting their Tanzanian retailers (domestic B2C flows)')
households.select_suppliers(G, firm_list, mode='inputed')

logging.info(
    'Tanzanian exporters are being selected by purchasing countries (export B2B flows)')
logging.info('and trading countries are being connected (transit flows)')
for country in country_list:
    country.select_suppliers(G, firm_list, country_list,
...
if export['sc_network_summary']:
    export_sc_netwrk_summary(G, firm_list, exp_folder)
...
logging.info(
    'The supplier--buyer graph is being connected to the transport network')
logging.info(
    'Each B2B and transit edge is being linked to a route of the transport network')
...
for country in country_list:
    country.decide_initial_routes(
        G, T, transport_modes, account_capacity, monetary_units_in_model)
...
for firm in firm_list:
    if firm.sector_type not in ['services', 'utility', 'transport']:
        firm.decide_initial_routes(
            G, T, transport_modes, account_capacity, monetary_units_in_model)
logging.info(
    'The supplier--buyer graph is now connected to the transport network')
...
if disruption_analysis is None:
    ...
    if export['district_sector_table']:
        exportDistrictSectorTable(
            filtered_district_sector_table, export_folder=exp_folder)
    ...
    disruption_list = def_dirspt_list(disruption_analysis, transport_network=T,
    ...
    if export['criticality']:
        criticality_export_file = initializeCriticalityExportFile(
            export_folder=exp_folder)
    ...
    for disruption in disruption_list:
        ...
        if disruption == disruption_list[0]:
            ...
            if export['edgelist_table']:
                exportEdgelistTable(supply_chain_network=G,
                                    export_folder=exp_folder)
        ...
        logging.info("Time loop completed, {:.02f} min".format(
            computation_time/60))
