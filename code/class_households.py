...
class Households(object):
    ...
    def reset_variables(self):
        ...
        self.consumption_loss_per_sector = {
            key: 0 for key, val in self.final_demand_per_sector.items()}
    ...
    def select_suppliers(self, graph, firm_list, mode='inputed'):
        if mode == 'equal':
            for firm in firm_list:
                ...
                self.purchase_plan = {
                    firm.pid: self.budget/len(firm_list) for firm in firm_list}
            ...
            dic_sector_to_nbfirms = firm_id_each_sector.groupby(
                'sector')['firm'].count().to_dict()
            sectors = firm_id_each_sector['sector'].unique()
            consumption_each_sector_quantity = {
                sector: self.budget/len(sectors) for sector in sectors}

            for sector in sectors:
                nb_retailers = random.randint(1, dic_sector_to_nbfirms[sector])
                retailers = random.sample(
                    firm_id_each_sector.loc[firm_id_each_sector['sector'] == sector, 'firm'].tolist(), nb_retailers)
            ...
            if len(self.purchase_plan) == 0:
                ...
                raise KeyError(
                    'Households: mode==inputed but no final_demand_per_sector')
            else:
                for retailer_id, purchase_this_retailer in self.purchase_plan.items():
                    if purchase_this_retailer > 0:
                        ...
                        weight = purchase_this_retailer / \
                            self.final_demand_per_sector[sector]
                        graph[firm_list[retailer_id]][self]['weight'] = weight
                        self.retailers[retailer_id] = {
                            'sector': self.pid, 'weight': weight}
                        firm_list[retailer_id].clients[self.pid] = {
