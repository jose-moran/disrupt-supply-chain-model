...
class Firm(object):
    ...
    def __init__(self, pid, odpoint=0, sector=0, sector_type=None, input_mix=None, target_margin=0.2, utilization_rate=0.8,
        ...
        if input_mix is None:
            ...
            self.inventory_duration_target = {
                key: inventory_duration_target for key in input_mix.keys()}
        ...
        self.eq_finance = {"sales": 0, 'costs': {
            "input": 0, "transport": 0, "other": 0}}
        self.eq_profit = 0
        self.eq_price = 1
    ...
    def initialize_fin_var_using_eq_cost(self, eq_production, eq_input_cost,
        ...
        self.eq_profit = self.eq_finance['sales'] - \
            sum(self.eq_finance['costs'].values())
    ...
    def add_noise_to_geometry(self, noise_level=1e-5):
        self.geometry = shapely.geometry.point.Point(
            self.long+noise_level*random.uniform(0, 1), self.lat+noise_level*random.uniform(0, 1))
    ...
    def select_suppliers(self, graph, firm_list, country_list,
        ...
        firm_list : list of Firms
            Generated by create_firms function
        country_list : list of Countries
            Generated by create_countriesfunction
        ...
        for sector_id, sector_weight in self.input_mix.items():
            ...
            # calculate their probability to be chosen, based on distance and importance
                ...
                # Identify the id of potential suppliers among the other firms
                potential_supplier_pid = [
                    firm.pid for firm in firm_list if firm.sector == sector_id]
                ...
                importance_of_each = rescale_values(
                    [firm_list[firm_pid].importance for firm_pid in potential_supplier_pid])  # Get importance for each of them
                prob_to_be_selected = np.array(
                    importance_of_each) / (np.array(distance_to_each)**weight_localization)
                prob_to_be_selected /= prob_to_be_selected.sum()
            ...
            # Generate one random weight per number of supplier, sum to 1
            ...
            for supplier_id in selected_supplier_id:
                ...
                if sector_id == import_code:
                    supplier_object = [
                        country for country in country_list if country.pid == supplier_id][0]
                ...
                supplier_object.clients[self.pid] = {
                    'sector': self.sector, 'share': 0, 'share_transport': 0}
    ...
    def choose_route(self, transport_network,
        ...
        if possible_transport_modes == "intl_multimodes":
            ...
            modes = ['intl_road_shv', 'intl_road_vnm',
                     'intl_rail', 'intl_river']
            routes = {
            ...
            if len(modes_weight) == 0:
                logging.warning(
                    "All transport modes are over capacity, no route selected!")
                return None
            # and select one route choosing random weighted choice
            selection_weights = rescale_values(
                list(modes_weight.values()), minimum=0, maximum=0.5)
    ...
    def decide_initial_routes(self, graph, transport_network, transport_modes,
        ...
        for edge in graph.out_edges(self):
            if edge[1].pid == -1:  # we do not create route for households
                continue
            # we do not create route for service firms if explicit_service_firms = False
            elif edge[1].odpoint == -1:
                continue
            else:
                ...
                cond_from = (transport_modes['from']
                             == "domestic")  # self is a firm
                ...
                transport_mode = transport_modes.loc[cond_from &
                                                     cond_to, "transport_mode"].iloc[0]
                ...
                if account_capacity:
                    ...
                    new_load_in_tons = transformUSDtoTons(
                        new_load_in_usd, monetary_unit_flow, self.usd_per_ton)
                    transport_network.update_load_on_route(
                        route, new_load_in_tons)

    def calculate_client_share_in_sales(self):
        # Only works if the order book was computed
        self.total_order = sum(
            [order for client_pid, order in self.order_book.items()])
        self.total_B2B_order = sum(
            [order for client_pid, order in self.order_book.items() if client_pid != -1])
        for client_pid, info in self.clients.items():
            ...
            if self.total_B2B_order == 0:
                ...
                if client_pid == -1:
                    ...
                    info['share_transport'] = self.order_book[client_pid] / \
                        self.total_B2B_order
    ...
    def calculate_price(self, graph, firm_list):
        ...
        if self.check_if_supplier_changed_price(graph, firm_list):
            ...
            for edge in graph.in_edges(self):
                eq_unitary_input_cost += graph[edge[0]
                                               ][self]['object'].eq_price * graph[edge[0]][self]['weight']
                est_unitary_input_cost_at_current_price += graph[edge[0]
                                                                 ][self]['object'].price * graph[edge[0]][self]['weight']
    ...
    def decide_purchase_plan(self, mode="equilibrium"):
        ...
        self.current_inventory_duration = {
            input_id: (evaluate_inventory_duration(
                ref_input_needs[input_id], stock) if input_id in ref_input_needs.keys() else 0)
                ...
                if inventory_duration is not None:
                    if inventory_duration < 1 - 1e-6:
                        if -1 in self.clients.keys():
                            sales_to_hh = self.clients[-1]['share'] * \
                                self.production_target
                        else:
                            sales_to_hh = 0
                        logging.debug('Firm '+str(self.pid)+" of sector "+str(self.sector)+" selling to households "+str(
        ...
        purchase_plan_per_sector = {
            input_id: purchase_planning_function(
                need, self.inventory[input_id], self.inventory_duration_target[input_id], self.reactivity_rate)
        ...
        self.purchase_plan = {
            supplier_id: purchase_plan_per_sector[info['sector']
                                                  ] * info['weight']
        ...
        for edge in graph.in_edges(self):
            if edge[0].pid in self.purchase_plan.keys():
                ...
                if quantity_to_buy == 0:
                    logging.debug(
                        "Firm "+str(self.pid)+": I am not planning to buy anything from supplier "+str(edge[0].pid))
            else:
                logging.error("Firm "+str(self.pid)+": supplier " +
                              str(edge[0].pid)+" is not in my purchase plan")
                quantity_to_buy = 0
    ...
    def produce(self, mode="Leontief"):
        max_production = production_function(
            self.inventory, self.input_mix, mode)
        self.production = min(
            [max_production, self.production_target, self.production_capacity])
        self.product_stock += self.production
        if mode == "Leontief":
            input_used = {input_id: self.production *
                          mix for input_id, mix in self.input_mix.items()}
            self.inventory = {
                input_id: quantity - input_used[input_id] for input_id, quantity in self.inventory.items()}
        else:
    ...
    def calculate_input_induced_price_change(self, graph):
        ...
        for edge in graph.in_edges(self):
            eq_theoretical_input_cost += graph[edge[0]
                                               ][self]['object'].eq_price * graph[edge[0]][self]['weight']
            current_theoretical_input_cost += graph[edge[0]
                                                    ][self]['object'].price * graph[edge[0]][self]['weight']
        input_cost_share = eq_theoretical_input_cost / 1
        relative_change = (current_theoretical_input_cost -
                           eq_theoretical_input_cost) / eq_theoretical_input_cost
        return relative_change * input_cost_share / (1 - self.target_margin)

    # firms could record the last price they paid their input
    def check_if_supplier_changed_price(self, graph, firm_list):
        for edge in graph.in_edges(self):
    ...
    def deliver_products(self, graph, transport_network=None,
        ...
        if self.rationing > 1 + 1e-6:
            ...
            quantity_to_deliver = {
                buyer_id: order for buyer_id, order in self.order_book.items()}

        else:
            logging.debug('Firm '+str(self.pid)+': I have to ration my clients by ' +
                          '{:.2f}'.format((1-self.rationing)*100)+'%')
            # Evaluate the quantity to deliver to each buyer
            if rationing_mode == "equal":
                quantity_to_deliver = {
                    buyer_id: order * self.rationing for buyer_id, order in self.order_book.items()}
                ...
                if -1 not in self.order_book.keys():  # no household orders to this firm
                    ...
                    if order_households < self.product_stock:
                        ...
                        if (self.total_order-order_households) <= 0:
                            logging.warning(
                                "Firm "+str(self.pid)+': '+str(self.total_order-order_households))
                        rationing_for_business = remaining_product_stock / \
                            (self.total_order-order_households)
                        quantity_to_deliver = {
                            buyer_id: order * rationing_for_business for buyer_id, order in self.order_book.items() if buyer_id != -1}
                        quantity_to_deliver[-1] = order_households
                    else:
                        quantity_to_deliver = {
                            buyer_id: 0 for buyer_id, order in self.order_book.items() if buyer_id != -1}
        ...
        for edge in graph.out_edges(self):
            ...
            graph[self][edge[1]]['object'].delivery_in_tons = \
                transformUSDtoTons(
                    quantity_to_deliver[edge[1].pid], monetary_unit_flow, self.usd_per_ton)

            if explicit_service_firm:
                # If the client is B2C
                if edge[1].pid == -1:
                    self.deliver_without_infrastructure(
    ...
    def send_shipment(self, commercial_link, transport_network,
        ...
        if self.check_route_avaibility(commercial_link, transport_network, 'main') == 'available':
            ...
            self.tonkm_transported += commercial_link.delivery_in_tons * \
        ...
        if route is not None:
            ...
            if cost_repercussion_mode == "type1":  # relative cost change with actual bill
                ...
                new_transport_bill = commercial_link.delivery_in_tons * \
                    commercial_link.alternative_route_cost_per_ton
                normal_transport_bill = commercial_link.delivery_in_tons * \
                    commercial_link.route_cost_per_ton
                relative_cost_change = max(
                    new_transport_bill - normal_transport_bill, 0)/normal_transport_bill
                ...
                added_costUSD_per_mUSD = added_costUSD_per_ton / \
                ...
                relative_price_change_transport = \
                    commercial_link.price / \
                    (commercial_link.eq_price + self.delta_price_input) - 1
                if (commercial_link.price is None) or (commercial_link.price is np.nan):
                    raise ValueError(
                        "Price should be a float, it is "+str(commercial_link.price))
                ...
                logging.debug('Firm '+str(self.pid) +
                              ...
                              " increased bill mUSD " +
                              str(added_costmUSD_per_mUSD *
                                  commercial_link.delivery)
                              )
        ...
        if len(transport_network.congestionned_edges) > 0:
            ...
            for edge in graph.out_edges(self):
                if graph[self][edge[1]]['object'].current_route == 'main':
                    ...
                    route_to_check = graph[self][edge[1]
                                                 ]['object'].alternative_route
                else:
                    continue
                ...
                if len(set(route_to_check) & set(transport_network.congestionned_edges)) > 0:
                    ...
                    self.generalized_transport_cost += transport_network.giveCongestionCostOfTime(
                        route_to_check)
        ...
        if len(transport_network.congestionned_edges) > 0:
            ...
            for edge in graph.out_edges(self):
                ...
                if len(set(route_to_check) & set(transport_network.congestionned_edges)) > 0:
                    ...
                    actual_route_time_cost = transport_network.giveRouteCostWithCongestion(
                        route_to_check)
                    ...
                    if graph[self][edge[1]]['object'].current_route == 'main':
                        ...
                        relative_cost_change_with_congestion = (
                            actual_route_time_cost - graph[self][edge[1]]['object'].route_time_cost)/graph[self][edge[1]]['object'].route_time_cost
                        self.finance['costs']['transport'] += self.eq_finance['costs']['transport'] * \
                    ...
                    # Otherwise, we need to incremen the added price increase due to transport
                        ...
                        relative_cost_change_no_congestion = (graph[self][edge[1]]['object'].alternative_route_time_cost -
                                                              graph[self][edge[1]]['object'].route_time_cost)/graph[self][edge[1]]['object'].route_time_cost
                        relative_cost_change_with_congestion = (
                            actual_route_time_cost - graph[self][edge[1]]['object'].route_time_cost)/graph[self][edge[1]]['object'].route_time_cost

                    # We increment financial costs
                    self.finance['costs']['transport'] += self.eq_finance['costs']['transport'] * self.clients[edge[1].pid]['share'] * (
                        1 + relative_cost_change_with_congestion - relative_cost_change_no_congestion)

                    # We compute the new price increase due to transport and congestion
                    relative_price_change_transport_no_congestion = self.eq_finance['costs']['transport'] * relative_cost_change_no_congestion / (
                        (1-self.target_margin) * self.eq_finance['sales'])
                    relative_price_change_transport_with_congestion = self.eq_finance['costs']['transport'] * relative_cost_change_with_congestion / (
                        (1-self.target_margin) * self.eq_finance['sales'])
                    total_relative_price_change = self.delta_price_input + \
                        relative_price_change_transport_with_congestion
                    new_price = graph[self][edge[1]]['object'].eq_price * \
                        (1 + total_relative_price_change)
                    logging.debug('Firm '+str(self.pid)+": price of transport to "+str(edge[1].pid)+"is impacted by congestion. New price: "+str(new_price)+" vs. "+str(graph[self][edge[1]]['object'].price)+". Route cost with congestion: "+str(actual_route_time_cost)+" vs. normal: "+str(
                        graph[self][edge[1]]['object'].route_time_cost)+", delta input: "+str(self.delta_price_input)+",  delta transport no congestion: "+str(relative_price_change_transport_no_congestion)+", delta congestion:"+str(relative_price_change_transport_with_congestion - relative_price_change_transport_no_congestion))
                    graph[self][edge[1]]['object'].price = new_price

                    # Retransfer shipment
                    transport_network.transport_shipment(
                        graph[self][edge[1]]['object'])
    ...
    def check_route_avaibility(self, commercial_link, transport_network, which_route='main'):
        ...
        if which_route == 'main':
            ...
            KeyError(
                'Wrong value for parameter which_route, admissible values are main and alternative')
        ...
        for edge in graph.in_edges(self):
            if explicit_service_firm == True:
                if graph[edge[0]][self]['object'].product_type in ['services', 'utility', 'transport']:
                    ...
                    self.receive_shipment_and_pay(
                        graph[edge[0]][self]['object'], transport_network)
            else:
    ...
    def evaluate_profit(self, graph):
        ...
        expected_gross_margin_no_transport = 1 - \
            sum(list(self.input_mix.values()))
    ...
    def print_info(self):
        ...
        print("order_book:", self.order_book,
              "; total_order:", self.total_order)
        print("input_needs:", self.input_needs)
        print("purchase_plan:", self.purchase_plan)
        print("inventory:", self.inventory)
        print("production:", self.production, "; production target:",
              self.production_target, "; product stock:", self.product_stock)
